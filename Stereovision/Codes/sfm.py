...
def compute_fundamental(x1, x2):
    """ Computes the fundamental matrix from a corresponding points
    ...
    for i in range(n):
        ...
        #A[i] = [x1[0, i]*x2[0, i], x1[1, i]*x2[0, i], x2[0, i], x1[0, i]*x2[1, i], x1[1, i]*x2[1, i], x2[1, i], x1[0, i], x1[1, i], 1]
...
def H_norm(x):
    """ Returns the matrix used for the normalization of the x points
     in the normalized 8 point algorithm. """

    x = x / x[2]
    mean = np.mean(x[:2], axis=1)
...
def skew(a):
    """ Skew a matrix A such that a x v = Av for any v. """
...
def F_from_ransac(x1, x2, model, maxiter=10000, match_threshold=1e-6):
